...
public class CountryRepository : IDisposable
{
    ...
    public CountryRepository(IConfiguration configuration, IDistributedCache cache)
    {
        strConnString = configuration.GetConnectionString("ZarenTravel");
        connection = new SqlConnection(strConnString);
        _cache = cache;
    }
    public List<Country> GetAllByCache()
    {
        try
        {
            var encKey = "CountryGetAllByCache";
            if (_cache.GetString(encKey) == null)
            {
                var list = connection.Query<Country>("CountryGetAll", commandType: CommandType.StoredProcedure).ToList();
                _cache.SetString(encKey, list.ToJson());
                return list;
            }
            else
            {
                var existingList = JsonConvert.DeserializeObject<List<Country>>(_cache.GetString(encKey));
                return existingList;
            }
        }
        catch (Exception ex)
