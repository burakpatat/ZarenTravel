...
function checkEmail(selector) { 
    if (selector === "") {
        ShowWarning("error.null");
        return false;
    }
   else if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(selector)) {
        return (true)
    }
...
function validText(selector) { 
    if (selector === "") {
        ShowWarning("error.null"); 
        return false;
    }
    else if (/^[a-zA-Z0-9!@#\$%\^\&*\)\(+=._-]+$/g.test(selector)) {
        return (true)
    }  
    ShowWarning("error.name.surname"); 
    return (false)
}
